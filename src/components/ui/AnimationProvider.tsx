...
function initSmoothScroll() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (window as any).locomotiveScroll = new LocomotiveScroll({
    lenisOptions: {
      wrapper: window,
  ...
  (function applyProxy() {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const lenis = (window as any).locomotiveScroll?.lenisInstance;
  ...
  track.addEventListener('click', (e) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).locomotiveScroll.scrollTo(target, { immediate: false, duration: 0.6 });
  });
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).locomotiveScroll.scrollTo(target, { immediate: true });
  });
...
function initNavManager() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { ANIM: anim, shuffleArray } = (window as any).animUtils;
  ...
  if (toggleBtn && menu) {
    ...
    document.addEventListener('keydown', (ev: KeyboardEvent) => { if (ev.key === 'Escape' && isOpen) closeMenu(); });
